...
class SimplifiedCryptoStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.lookback           = 48
        self.sqrt_annualization = np.sqrt(12 * 24 * 365)  # 12 five-minute bars per hour
    ...
    def _initialize_symbol(self, symbol):
        self.crypto_data[symbol] = {
            ...
            'vwap': 0.0,
            'volume_long': deque(maxlen=288),  # 288 five-min bars = 24 hours
            'vwap_sd': 0.0,
            'vwap_sd2_lower': 0.0,
            'vwap_sd3_lower': 0.0,
    ...
    def _update_market_context(self):
        # With 5-min bars, use 96-bar SMA (8 hours) for regime detection.
        # Previously 48 1-min bars (48 min) — too short for meaningful regime.
        regime_lookback = 96
        if len(self.btc_prices) >= regime_lookback:
            ...
            btc_sma = np.mean(btc_arr[-regime_lookback:])
