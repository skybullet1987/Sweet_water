...
class MicroScalpEngine:
    ...
    Signals
    -------
    1. Order Book Imbalance (OBI): bid/ask pressure - LIVE ONLY (synthetic data unreliable in backtest)
    ...
    6. CVD Divergence: absorption at support - LIVE ONLY (approximated from OHLCV, unreliable in backtest)
    """
    ...
    def calculate_scalp_score(self, crypto):
        ...
        try:
            ...
            # DISABLED in backtest: CVD is approximated from OHLCV, unreliable.
            # ----------------------------------------------------------
            if self.algo.LiveMode:
                cvd = crypto.get('cvd')
        ...
        # In live: OBI enabled — no microstructure → cap at 0.50; full → uncapped.
        # In backtest: OBI disabled — gate only on volume ignition.
        if self.algo.LiveMode:
